...
if __name__ == '__main__':
    ...
    straightConst = 85 				#angle in which car goes straight
    angle = 85 						#variable for the current angle of the car
    prevAngle = angle				#variable tracking the angle of the previous iteration
    
    speed = 60 #variable for initial speed of the car
    reverseSpeed = 40 #variable for speed of the car going backwards
...
#--------------------------------{ main loop }---------------------------------------#
    while True:
        ...
        if cPillar.area > 0 and cPillar.y > 350 and not lapsComplete:
        ...
        elif cPillar.area > 0 and cPillar.y > 300 and not lapsComplete:
            ...
            if distanceC is not None and distanceC < 100:
                ...
                #drive_straight_until_obstacle(pca, imu_sensor, target_heading, center_sensor, stop_distance=15)
            
                perform_reverse_turn_to_heading(pca, imu_sensor, turnDir, target_heading)
                t += 1
                target_heading = decide_target_heading(t, turnDir)
