...
left_sensor = setup_sensor(0)  # Left side sensor
...
# === PID Settings ===
Kp = 0.1
ideal_distance = 150  # mm from wall

# === Main Loop ===
try:
    while True:
        select_channel(i2c, 0)
        distance = left_sensor.range
        error = distance - ideal_distance  # +ve if too far

        correction = Kp * error
        new_angle = 90 - correction  # subtract to turn left if too far

        set_servo_angle(new_angle)

        print(f"Left: {distance} mm | Error: {error:.2f} | Servo: {new_angle:.2f}°")
